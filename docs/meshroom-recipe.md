...
| Mesh upside down after georef: hoop houses are troughs, trees hang below ground, north mirrored in the ortho (Godot shows it inside-out; Blender and the ortho hide it) | AliceVision writes OBJ/Alembic in the graphics frame (Y-up, Z-back) but `cameras.sfm` is in the CV frame (Y-down, Z-forward) — the mesh is the solve rotated 180° about X. Cameras on one flight plane fit either way, so `georef_solve.py --apply` tests identity + the three 180° rotations and keeps the one whose faces point at the cameras (prints `mesh frame vs solve: … -> using rot180_x`); ODM meshes pick identity. Verified on the farm: ground lands at 28.2 m ellipsoid = RTK takeoff height, trees +30 m |
