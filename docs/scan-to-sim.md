...
- Firmware-in-the-loop: PX4 SITL + Gazebo is the standard; **Betaflight SITL is verified real and documented** ([official docs](https://betaflight.com/docs/development/SITL)): builds from the main repo (`make TARGET=SITL`), the 2025.12 Configurator connects over websockets, and it bridges to external simulators — **Gazebo via bidirectional UDP (ports 9002/9003)** and RealFlight. (Known quirks: WSL IP churn, UARTs as local TCP.) **Applicability note:** the mapping drone is a DJI Mini 3 (proprietary firmware — no SITL of any kind), so firmware-in-the-loop only matters for the future custom FPV-arm build, *if* that build ends up on Betaflight; PX4 SITL remains the alternative. Decide when that airframe's flight controller is chosen.
